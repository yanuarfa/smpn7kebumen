    ...
    <div style="min-height: 100vh; position: relative;">
        <section class="w-full">
            @if ($sampul->count() > 0)
                    ...
                    <div class="relative h-[200px] overflow-hidden sm:h-[300px] md:h-[400px] lg:h-[600px] group">
                        @foreach ($sampul as $s)
                            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                                <img src="{{ asset('storage/' . $s->file_sampul) }}"
                                    class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
                                    alt="{{ $s->deskripsi_sampul }}">
                            </div>
                        @endforeach
                        @if ($sampul->count() !== 1)
                            <button type="button"
                ...
                @else
                    <img src="{{ asset('css/main/img/smpn7kebumen.jpeg') }}" alt="banner"
                    ...
                    @if ($kepalasekolah)
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div class="place-self-center md:place-self-start p-6 border col-span-1 max-w-[400px]">
                                ...
                                <h3 class="text-center">{{ $kepalasekolah->nama }}</h3>
                                <p class="text-center">{{ $kepalasekolah->jabatan }}</p>
                            </div>
                ...
                <div>
                            ...
                            @foreach ($berita as $item)
                                <div
                                    ...
                                    <h3 class="pb-2"><a class="color-default line-clamp-2"
                                            href="{{ route('detail-berita', $item->slug) }}">{{ $item->title }}</a></h3>
                                    ...
                                    <div class="flex justify-between items-center pt-3">
                                        <ul class="flex gap-2">
                                            ...
                                            <li><a href="#" class="color-default"><i class="fa fa-tags"
                                                        aria-hidden="true"></i>{{ $item->category->name }}</a>
                                            </li>
                                        </ul>
                                        <p class="pb-2">{{ $item->created_at->format('d F Y') }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    @else
                        <div class="flex items-center justify-center flex-col pt-6">
                            <img src="{{ asset('css/main/img/no-data.svg') }}" class="h-[300px]">
                    ...
                    <div class="rc-carousel" data-smart-speed="2000" data-autoplay="true" data-dots="true">
                        @foreach ($pengajar as $item)
                            <div class="p-4 bg-white h-[350px] max-w-[200px]">
                                ...
                                <h4 class="text-center line-clamp-2">{{ $item->nama }}</h4>
                                <p class="text-center">Guru {{ $item->bidang }}</p>
                                <div class="flex items-center flex-col">
                                    <ul class="flex gap-2 pt-2 *:bg-yellow-400 *:px-2 *:py-1">
                                        <li><a href="" target="_blank"><i class="fa fa-globe"
                                                    aria-hidden="true"></i></a>
                                        </li>
                                        <li><a href="mailto:{{ $item->email }}"><i class="fa fa-envelope-o"
                                                    aria-hidden="true"></i></a>
                                        </li>
                                    </ul>
                                </div>
